...
export class DeploymentsProcessor {
  ...
  async process(id: string) {
    ...
    try {
      ...
      const variables =
        typeof deploy.envVariables === "string"
      ...
      this.logger.error(
        `[CRITICAL ERROR] Deploy ${id} failed: ${error.message}`,
      );
      ...
      if (fs.existsSync(workDir)) {
        try {
          ...
          this.logger.error(
            `[CLEANUP ERROR] Could not delete ${workDir}: ${cleanupError.message}`,
          );
        }
      }
    }
